...
function pointToLinePerpendicularDistance(point:any, lineStart:any, lineEnd:any) {
    const dx = lineEnd.x - lineStart.x;
    const dy = lineEnd.y - lineStart.y;
    const dz = lineEnd.z - lineStart.z;
  
    const numerator = Math.abs((point.y - lineStart.y) * dx - (point.x - lineStart.x) * dy + (point.z - lineStart.z) * dz);
...
export function thumbFingerStraight(landmarks: any[]): boolean {
    ...
    if(distance56>0.05){
        ...
        const perpendicularDistance = pointToLinePerpendicularDistance(tip, landmark5, landmark6);
        ...
        const openThresholdRatio = 0.70 + (1.05 - 0.70) * zRatio; // 0.70 ~ 1.05
...
export function isHandBent(landmarks: any[]): boolean {
    ...
    return thumbIsBent&&indexIsBent && middleIsBent && ringIsBent && pinkyIsBent;
